...
// TestMCPServerDiscovery_DrainSetsDeletingStatus mirrors the ModelDiscovery
// drain-status fix for MCPServerDiscovery: while the finalizer waits for
// owned children to drain, one reconcile must flip the parent's Ready
// condition to False/Deleting (and requeue). Fake client with a
// finalizer-pinned child keeps the drain window open so the assertion is
// deterministic (envtest drains children in milliseconds).
func TestMCPServerDiscovery_DrainSetsDeletingStatus(t *testing.T) {
	...
	child := &litellmv1alpha1.LiteLLMMCPServer{
		ObjectMeta: metav1.ObjectMeta{
			Name:       "big-mcp-provider.server-0001",
			Namespace:  "default",
	...
	r := &MCPServerDiscoveryReconciler{
		Client:    cli,
		Scheme:    scheme,
