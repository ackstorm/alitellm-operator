...
- **`LiteLLMA2AAgent.spec.exposeAsModel`** — replaced by
  ...
  your agents and add an a2a Discovery (same `agent.<name>` names with
  `prefix: agent`). Children generated by an older operator are not pruned
  after the upgrade; delete them with
  `kubectl delete litellmmodel -l litellm.ackstorm.ai/generated-by-agent`.

### Fixed
- **ModelDiscovery took over a name held by a deleting model only after a full
  refresh interval.** It now retries every 5s until the deleting model is gone.
