...
- **LiteLLM chart `1.84.0`** has default `appVersion: v1.84.0`. The operator (this repo) targets `v1.83.10-stable` per auto-memory `litellm_version_target.md`. `test/e2e/cluster/01-deps/litellm.values.yaml` overrides `image.tag` accordingly. A Tier 2 scope assertion (AC-N1+N2, Task 9.11) verifies the rendered LiteLLM Deployment actually uses the overridden tag.
