...
This document describes the release process for the alitellm-operator, including how to create releases, what happens during the release workflow, and how to handle the automated updates.

## Overview

The alitellm-operator uses a fully automated release process that:
...
- Helm chart files (`deploy/charts/alitellm-operator/**`, `helm/**`)
...
- **Docker Images**: 
  - `ghcr.io/ackstorm/alitellm-operator:{version}` (multi-arch: amd64, arm64)
  - `ghcr.io/ackstorm/alitellm-operator:latest` (multi-arch: amd64, arm64)
...
- **Helm Chart**: Published to `ghcr.io/ackstorm/charts/alitellm-operator`
...
2. **Helm Chart Version**: `deploy/charts/alitellm-operator/Chart.yaml`
3. **Helm Chart AppVersion**: `deploy/charts/alitellm-operator/Chart.yaml`
...
helm install alitellm-operator oci://ghcr.io/ackstorm/charts/alitellm-operator --version v1.0.0
```
...
kubectl apply -k https://github.com/ackstorm/alitellm-operator/config/crd

# Install the operator using kustomize
kubectl apply -k https://github.com/ackstorm/alitellm-operator/config/default
```
...
4. Check that the OCI registry path is correct (`ghcr.io/ackstorm/charts/`)
...
3. **Adjust Helm chart settings** in `deploy/charts/alitellm-operator/Chart.yaml`
...
1. Check the [GitHub Actions logs](https://github.com/ackstorm/alitellm-operator/actions)
