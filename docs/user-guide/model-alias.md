...
A `LiteLLMModelDiscovery` with `spec.aliasSuffix` writes its own alias CRs
(`<discovery>-aliases-<n>`). They are ordinary alias CRs — same map, same
conflict rules — but owned by the discovery: edit the discovery, not them.
See [Default aliases](model-discovery.md#default-aliases-specaliassuffix).
