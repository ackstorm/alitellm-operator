...
| `aliasSuffix` _string_ | AliasSuffix, when set, makes the Discovery emit one<br />router_settings.model_group_alias entry `<child><suffix> → <child>`<br />per generated child — e.g. "[1m]" so Claude Code's 1M-context id<br />`claude-opus-5-5[1m]` resolves to the discovered `claude-opus-5-5`.<br />Entries land in operator-owned LiteLLMModelAlias CRs named<br />`<discovery>-aliases-<n>` (128 entries each) and are removed when the<br />suffix is cleared. Charset mirrors LiteLLMModelAlias entry names. |  | MaxLength: 32 <br />Pattern: `^[A-Za-z0-9._:/@+\[\]-]+$` <br /> |
